...
def call_openai_for_analysis(image_info, folder_out, multiple_folders) -> list[dict]:
    system_prompt = """
...
def analyze_images_in_folder(folder_in: str, folder_out: str, multiple_folders: bool) -> list[dict]:
    image_info = []
    for root, dirs, files in os.walk(folder_in):
        for file_pattern in ["*.jpg", "*.jpeg", "*.png", "*.tiff"]:  # Add other relevant patterns
    ...
    folder_out_substructue = "\n".join(glob(os.path.join(folder_out, "**/*"), recursive=True))
    return call_openai_for_analysis(image_info, folder_out_substructue, multiple_folders)

if __name__ == "__main__":
    folders = analyze_images_in_folder("/Users/julius/projects/image-sorter/test_in", "/Users/julius/projects/image-sorter/test_out", True)
    print(folders)
